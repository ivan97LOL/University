...
valorpruebasa=0.3  # Valor de las pruebas del autocorrector

npruebasc=$(ls -1 pruebas/z* 2>/dev/null | wc -l)  # Número de pruebas del corrector final
valorpruebasc=0.79  # Valor de las pruebas del corrector final
...
g++ -Wall -g -std=c++11 $FUENTE -o $PRINCIPAL 2> $tempfile
if [ $? -ne 0 ]; then
        cat $tempfile
